## ukb_ancestryplot.R for KING Ancestry plot, by Zhennan Zhu and Wei-Min Chen
rm(list=ls(all=TRUE))
library(e1071)
library(dplyr)
options(scipen = 999)
...
para.one <- function(x) {
  ...
  mod.svm <- svm(svm.mod, data = dat[dat$fold != fold.index, ] %>% mutate(Population=as.factor(Population)), kernel = "radial", 
                 gamma = 10^para[x, "gamma"], cost = 10^para[x, "cost"], fitted = F, type="C")
...
for (i in 1:2) {
  ...
  mod_svm <- svm(formula = svm.mod, data = dat[index, ] %>% mutate(Population=as.factor(Population)), kernel = "radial", gamma = 10^best.set[1], 
                 cost = 10^best.set[2], probability = T, fitted = T, type="C")
